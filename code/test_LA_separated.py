...
from networks.vnet_symmetric_head_separated import VNetMultiHead,VNetMultiHeadRes
from networks.kiunet import kiunet3d
...
parser.add_argument('--model', type=str,  default='3DUNSMLNet_10200915_datacrop_random', help='model_name')
...
parser.add_argument('--format', type=str, default='nrrd', help='data format, h5 or nrrd')
...
parser.add_argument('--test_vnet', type=int, default=False, help='test VNet')
parser.add_argument('--test_kiunet', type=int, default=False, help='test MLNet')
#########
# save options
#########
...
if FLAGS.test_kiunet:
    patch_shape=(128, 128, 48)
else:
    patch_shape=(256, 256, 80)
num_classes = 2
output_size = (256,256,88)
#with open( '../data/alldata.list', 'r') as f:
# with open( '../data/test20.list', 'r') as f:
with open( '/home/LYS/MICCAI2018/data/3D/test54.list', 'r') as f:
    image_list = f.readlines()[10:25]

if FLAGS.format == 'h5':
    image_list = [FLAGS.root_path + item.replace('\n', '') + "/mri_norm_roicrop.h5" for item in image_list]
elif FLAGS.format == 'nrrd':
    # data_path = '/home/LYS/MICCAI2018/data/3D/All Volumes/'
    data_path = '/home/LYS/MICCAI2018/data/3D/Testing Set/'
...
def test_calculate_metric(epoch_num):
    if FLAGS.test_ml:
        ...
        has_enout = True
    if FLAGS.test_vnet:
        net =  VNet(n_channels=1, n_classes=num_classes, normalization='batchnorm', has_dropout=False).cuda()#.to(device)
    elif FLAGS.test_kiunet:
        FLAGS.step_xy = 64
        FLAGS.step_z = 8
        net = kiunet3d(c=1, n=16, dropout=0.3, norm='in', num_classes=num_classes).cuda()
        ...
        # net = VNetMultiHeadRes(n_channels=1, n_classes=num_classes, normalization=FLAGS.normalization,
        #                 ratio=FLAGS.ratio, has_att=FLAGS.attention, has_enout=has_enout, has_dropout=False).cuda()
...
def dist_test_all_case(net, image_list, num_classes, save_result=True, test_save_path=None, preproc_fn=True,has_post=False,save_un=False):
    ...
    for image_path in tqdm(image_list):
        ...
        with torch.no_grad():
            ...
            # prediction, score_map = test_single_case(net, image, num_classes=num_classes)
...
def test_single_case(net, image, num_classes=1):
    ...
    y_en_soft = F.softmax(y_decoder, dim=1)
    y = y_de_soft  # torch.mean(torch.stack([y_de_soft, y_en_soft]), dim=0)
    y = y.cpu().data.numpy()
    y = y[0, :, :, :, :]
    score_map = y
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                if FLAGS.test_ml or FLAGS.test_vnet or FLAGS.test_kiunet:
